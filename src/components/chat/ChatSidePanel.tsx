...
export const ChatSidePanel: React.FC<Props> = ({ contact, myUserId, token, onClose }) => {
  ...
  const pendingRef = useRef<ChatMessageData[]>([]); // ⬅️ buffer de mensajes “tempranos”
  ...
  const loadChat = async (mountedRef: { current: boolean }) => {
    ...
    // Reproducir mensajes que llegaron antes de tener chatId
    ...
    if (mountedRef.current) setMessages(cleaned);
  };
